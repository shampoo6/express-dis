...
export default class App {
    ...
    private readonly parserList: Function[]

    constructor() {
        this.staticDirMap = new Map<string, string>()
        this.parserList = new Array<Function>()
        this.e = express()
        App.ins = this
    }
    ...
    public async init() {
        ...
        global[GlobalKey.InjectMap] = new Map<any, Function>()
        ...
        let configFilePath = configPath ? path.resolve(process.cwd(), configPath) : path.resolve(process.cwd(), 'edis.config.js');
     ...
     * @param path 目录路径
     */
    public staticAssetsDir(url: string, path: string) {
        this.staticDirMap.set(url, path)
    }
     ...
     * @param middleware
     */
    public addParser(middleware: Function) {
